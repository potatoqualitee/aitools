...
Describe $CommandName -Tag UnitTests {
    Context "Validate parameters" {
        BeforeAll {
            ...
            [object[]]$params = $command.Parameters.Keys | Where-Object { $PSItem -notin ('WhatIf', 'Confirm') }
        ...
        It "Should only contain our specific parameters" {
            (@(Compare-Object -ReferenceObject ($knownParameters | Where-Object { $PSItem }) -DifferenceObject $params).Count ) | Should -Be 0
        }
    }
}
...
Describe $CommandName -Tag IntegrationTests {
    Context "Verifying Database is shrunk" {
        ...
        BeforeEach {
            ...
            $global:oldLogSize = $db.LogFiles[0].Size
            $global:oldDataSize = $db.FileGroups[0].Files[0].Size
            $db.Checkpoint()
        }
        AfterEach {
        ...
        It "Shrinks just the log file when FileType is Log" {
            ...
            $db.FileGroups[0].Files[0].Refresh()
            $db.LogFiles[0].Refresh()
            $db.FileGroups[0].Files[0].Size | Should -Be $global:oldDataSize
            $db.LogFiles[0].Size | Should -BeLessThan $global:oldLogSize
        }
        ...
        It "Shrinks just the data file(s) when FileType is Data" {
            ...
            $db.FileGroups[0].Files[0].Refresh()
            $db.LogFiles[0].Refresh()
            $db.FileGroups[0].Files[0].Size | Should -BeLessThan $global:oldDataSize
            $db.LogFiles[0].Size | Should -Be $global:oldLogSize
        }
